...
class GazeboWindPlugin : public ModelPlugin {
 ...
 protected:
  ...
  /// \brief Called when world is updated
  // updates wind field given a frequency
  void OnUpdateWind(const common::UpdateInfo& /*_info*/);
 ...
 private:
  ...
  std::string custom_wind_field_path_new;
  int num_custom_wind_fields;
  int current_custom_wind_field;
  float min_x_;
  float min_y_;
  int n_x_;
  int n_y_;
