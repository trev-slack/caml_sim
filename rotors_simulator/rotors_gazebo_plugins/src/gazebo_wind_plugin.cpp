...
void GazeboWindPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  if (!use_custom_static_wind_field_) {
    ...
    //get number of custom static wind fields
    getSdfParam<int>(_sdf, "numcustomwindfields", num_custom_wind_fields,
                        num_custom_wind_fields);
    custom_wind_field_path_new = custom_wind_field_path;
    current_custom_wind_field = 0;
  ...
  wind_field_update_frequency = 100;
  current_update = 0;
  update_wind_ = event::Events::ConnectWorldUpdateBegin(
      boost::bind(&GazeboWindPlugin::OnUpdateWind, this,_1));
}

//This gets called by the world update start event
void GazeboWindPlugin::OnUpdateWind(const common::UpdateInfo& _info) {
  //updating wind field every event start causes major sim-time hit. Instead update every x event
  if (wind_field_update_frequency > current_update) {
    //do not update
    current_update++;
    return;
  }
  //check if the current custom wind field number is outside the range
  if (num_custom_wind_fields<current_custom_wind_field) {
    current_custom_wind_field = 0;
  }
  //update the wind field path
  custom_wind_field_path_new.resize(custom_wind_field_path_new.size()-5);
  custom_wind_field_path_new = custom_wind_field_path_new + std::to_string(current_custom_wind_field) + ".txt";
  //read in the custom wind field
  ReadCustomWindField(custom_wind_field_path_new);
  //update the new number for wind field
  current_custom_wind_field = current_custom_wind_field + 1;
  current_update = 0;
}
